	...
	function Game() {
		...
		if (!Phaser.Device.desktop && window.innerWidth < 1000) {
			if (window.innerWidth < 700) {
				(0, _jquery2['default'])('.flex .ui-column').css('display', 'none');
			}
			_get(Object.getPrototypeOf(Game.prototype), 'constructor', this).call(this, window.innerWidth, window.innerHeight, Phaser.CANVAS, 'game');
		} else {
			_get(Object.getPrototypeOf(Game.prototype), 'constructor', this).call(this, 667, 375, Phaser.CANVAS, 'game');
		}
		...
		this.state.add('Intro', _statesIntro2['default'], false);
...
if (window.innerHeight > window.innerWidth && window.innerWidth < 700) {
	(0, _jquery2['default'])('.flex').css('display', 'none');
	(0, _jquery2['default'])('#turn').css('display', 'block');
} else {
	GENEVA = new Game();
	(0, _jquery2['default'])('#turn').css('display', 'none');
}
...
(0, _jquery2['default'])(window).on('resize', function () {
	if (window.innerHeight > window.innerWidth && window.innerWidth < 700) {
		if (GENEVA) {
			...
			(0, _jquery2['default'])('.flex').css('display', 'flex');
			(0, _jquery2['default'])('#turn').css('display', 'none');
		}
	} else {
		if (GENEVA) {
...
},{"./states/Intro":11,"ismobilejs":1,"jquery":2,"states/Boot":8,"states/GameOver":9,"states/GameTitle":10,"states/Main":12,"states/Preload":13}],4:[function(require,module,exports){
'use strict';
...
var Player = (function (_Phaser$Sprite) {
	...
	_createClass(Player, [{
		...
		value: function die() {
			...
			var text = this.game.add.text((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() / 2 - 60, "GAME OVER", {
...
var LifeManager = (function (_Phaser$Sprite) {
	_inherits(LifeManager, _Phaser$Sprite);

	function LifeManager(game, x, y, key, frame) {
		_classCallCheck(this, LifeManager);

		_get(Object.getPrototypeOf(LifeManager.prototype), "constructor", this).call(this, game, x, y, key, 3);
		this.game = game;
...
var Relique = (function (_Phaser$Sprite) {
	...
	function Relique(game, x, y, key, frame) {
		...
		this.phase1 = this.game.add.tween(this).to({
			y: [this.y - 300],
			x: this.x + 200
		}, 0.7 * Phaser.Timer.SECOND, Phaser.Easing.Cubic.Out);

		this.phase2 = this.game.add.tween(this.cameraOffset).to({
	...
	_createClass(Relique, [{
		...
		value: function loot() {
			if (!this.looted) {
				...
				_self.openPopup("Bravo ! Vous avez récupére l'accès aux personnes à mobilité réduite ! Une fois ramenée au Grand Conseil, tout le monde pourra accèder à la nouvelle salle !");
			}
		}
	}, {
		key: 'resume',
		value: function resume() {
			...
			if (this.game.paused && this.continueButton) {
				if (this.continueButton.getBounds().contains(this.game.input.x, this.game.input.y)) {
					(function () {
						_this.continueButton.setFrame(1);
						_this.box.kill();
						_this.continueButton.kill();
						_this.textPopup.kill();
						_this.body.checkCollision = false;
		...
		key: 'openPopup',
		value: function openPopup(text) {
			console.log(2);

			this.timerText = this.game.add.text((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() / 2, "3", {
			...
			this.box = this.game.add.image((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() / 2 - 30, 'intro-box', 0);
			this.box.fixedToCamera = true;
			this.box.anchor.set(0.5);
			this.continueButton = this.game.add.button((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() - 40, 'resume', null, null, null, null, 1, 0);
			...
			this.textPopup = this.game.add.text((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() / 2 - 30, text, {
...
},{"ismobilejs":1,"jquery":2}],9:[function(require,module,exports){
"use strict";
...
var GameTitle = (function (_Phaser$State) {
	...
	_createClass(GameTitle, [{
		...
		value: function create() {
			...
			this.background.width = (0, _jquery2['default'])('canvas').width();
			this.background.height = (0, _jquery2['default'])('canvas').height();
...
var Intro = (function (_Phaser$State) {
	...
	_createClass(Intro, [{
		...
		value: function create() {
			...
			var texts = ["Le bâtiment du grand Conseil, l’organe qui crée et valide les lois du canton, n’a pas été rénové depuis 1962.", "C’est pourquoi le Grand Conseil a décidé de faire appel à la Mère Royaume qu’ils jugent suffisamment courageuse pour relever un défi majeur. Elle aura besoin de ton aide pour récupérer des reliques, protégées par divers monstres, qui permettront de rénover la salle. ", "Le grand conseil est allé la chercher en train, dans son monde, directement en 1602. Elle arrive à la gare dans une ville qui a complétement changé depuis son époque et elle se retrouve un peu perdue…", "Aide la mère royaume à récupérer les reliques « durant ses péripéties à travers la ville ». Une fois que tu auras récupéré toutes les reliques tu pourras les amener à la salle du Grand Conseil qui pourra enfin être rénové."];
			var self = this;
			...
			var next = this.game.add.button(this.box.width / 2 - 100, this.box.height / 2 + 2, 'next', function () {
				self.game.sound.play('clic');
...
var _objectsLifeManager = require('../objects/lifeManager');
...
var Main = (function (_Phaser$State) {
	...
	_createClass(Main, [{
		...
		value: function create() {
			...
			this.player = new _objectsPlayer2['default'](this.game, 0, 600, 'mereRoyaume', 10, 100);
			...
			this.life = this.game.add.existing(new _objectsLifeManager2['default'](this.game, 45, 30, 'hearts', 3));
			this.life.fixedToCamera = true;
			this.life.bringToTop();
			var self = this;
			this.pauseButton = this.game.add.button(30, (0, _jquery2['default'])('canvas').height() - 30, 'pause', function () {
				self.pause(this);
			}, null, 1, 0, 1, 0);
		...
		value: function pause(state) {
			...
			if (this.game.paused == false) {
				self.game.sound.play('clic');
				self.timerText = this.game.add.text((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() / 2 - 20, "3", {
					font: 'bold 32pt perfectDOS',
					fill: 'black',
					align: 'left',
					wordWrap: true,
				...
				self.text = this.game.add.text((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() / 2 - 60, "PAUSE", {
					font: "40px perfectDOS",
					fill: "#FFFFFF",
					align: "right"
				});
				...
				self.text.fixedToCamera = true;
				this.resumeButton = self.game.add.button((0, _jquery2['default'])('canvas').width() / 2, (0, _jquery2['default'])('canvas').height() / 2, 'resume', function () {
					self.game.paused = false;
				}, null, null, null, 1, 0);
		...
		value: function resume() {
			...
			if (this.game.paused) {
				if (this.resumeButton.getBounds().contains(this.game.input.x, this.game.input.y)) {
					(function () {
						_this.resumeButton.setFrame(1);
						_this.resumeButton.destroy();
						_this.text.destroy();
...
var Preload = (function (_Phaser$State) {
	...
	_createClass(Preload, [{
		...
		value: function preload() {
			...
			this.splashScreen.width = (0, _jquery2['default'])('canvas').width();
			this.splashScreen.height = (0, _jquery2['default'])('canvas').height();
			this.loadingbar = this.game.add.sprite((0, _jquery2['default'])('canvas').width() / 2 - 100, (0, _jquery2['default'])('canvas').height() - 85, 'loadingbar');
		...
		key: 'loadComplete',
		value: function loadComplete(progress, cacheKey, success, totalLoaded, totalFiles) {
